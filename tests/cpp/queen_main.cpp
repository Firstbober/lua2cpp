...
luaValue _l2c__queen_export(luaState* state);

int main(int argc, char* argv[]) {
    std::cout << "Testing transpiled queen.lua..." << std::endl;
    ...
    if (argc <= 1) {
        args.push_back(luaValue(8));
        state.set_arg(args);
    }

    // Call the transpiled queen.lua module
    luaValue result = _l2c__queen_export(&state);
