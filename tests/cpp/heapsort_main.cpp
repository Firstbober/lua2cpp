...
luaValue _l2c__heapsort_export(luaState* state);

int main(int argc, char* argv[]) {
    std::cout << "Testing transpiled heapsort.lua..." << std::endl;
    ...
    if (argc <= 1) {
        args.push_back(luaValue(4));
        state.set_arg(args);
    }
    // Set default for arg[2]
    if (argc <= 2) {
        args.push_back(luaValue(10000));
        state.set_arg(args);
    }

    // Call the transpiled heapsort.lua module
